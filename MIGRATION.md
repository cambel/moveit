...
- Added an optional callback to the `TrajectoryExecutionManager::push()` methods. The callback receives the resulting `moveit_controller_manager::ExecutionStatus` when the execution is completed.
- Added an overload to the `TrajectoryExecutionManager::push()` method to allow the execution of a *sequence* of trajectories. Pushing a *sequence* of trajectories will block all the controllers needed by the entire *sequence* until the execution of the whole sequence is completed.
- Added an optional callback to the `TrajectoryExecutionManager::push()` method for a *sequence* of trajectories. The callback receives the index of each successfully completed trajectory segment (i.e. it is called multiple times).
